  ...
  setup(options, nuxt) {
    ...
    if (options.openapi) {
      ...
      nuxt.hook('nitro:config', (nitroConfig) => {
        nitroConfig.virtual = nitroConfig.virtual || {}
        nitroConfig.handlers = nitroConfig.handlers || []
        nitroConfig.virtual['#actions-openapi-registry'] = () => generateOpenApiRegistry(scannedActions, nuxt.options.buildDir)
        nitroConfig.virtual['#actions-openapi-handler'] = () => generateOpenApiHandler(info)
        nitroConfig.handlers.push({ route: jsonPath, method: 'get', handler: '#actions-openapi-handler' })
        if (uiPath) {
          nitroConfig.virtual['#actions-openapi-ui'] = () => generateSwaggerUi(jsonPath)
          nitroConfig.handlers.push({ route: uiPath, method: 'get', handler: '#actions-openapi-ui' })
        }
      })
    }
...
function generateOpenApiRegistry(actions: ScannedAction[], buildDir: string): string {
...
function generateOpenApiHandler(info: { title: string, version: string, description?: string }): string {
  return [
    `import { actions } from '#actions-openapi-registry'`,
    `export default defineEventHandler(async () => {`,
    `  const schemas = await collectSchemas(actions)`,
    `  return buildOpenApiDocument({ actions, schemas, info: ${JSON.stringify(info)} })`,
    `})`,
  ].join('\n')
}

function generateSwaggerUi(jsonPath: string): string {
